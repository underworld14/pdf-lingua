...
export async function POST(request: NextRequest) {
  try {
    ...
    await jobService.updateJobProgress(jobId, "PROCESSING" as JobStatus, "UPLOAD" as ProcessStep, 25);
...
async function processJob(jobId: string) {
  try {
    ...
    const jobService = new JobService();
    await jobService.updateJobProgress(jobId, "FAILED" as JobStatus, "UPLOAD" as ProcessStep, 0);
  }
}

// Functions for subsequent steps will be called by each previous step
async function extractAndTranslate(jobId: string, pdfService: PdfService, jobService: JobService) {
  try {
    // Get job from database
    const job = await jobService.getJob(jobId);
    if (!job) {
    ...
    const extractedContents: { fileId: string; filePath: string; htmlContent: string }[] = [];
    ...
    for (const file of job.files) {
      try {
        ...
        const htmlContent = await pdfService.extractTextFromPdf(file.originalPath);
        extractedContents.push({ fileId: file.id, filePath: file.originalPath, htmlContent });
      } catch (extractError) {
        console.error(`Error extracting from ${file.originalPath}:`, extractError);
      }
    }
    ...
    const jobService = new JobService();
    await jobService.updateJobProgress(jobId, "FAILED" as JobStatus, "EXTRACT" as ProcessStep, 50);
  }
}
  ...
  try {
    ...
    for (const { fileId, filePath, htmlContent } of extractedContents) {
      try {
        ...
        const translatedHtml = await pdfService.translateHtml(htmlContent, job.language);
        ...
        await jobService.updateFileWithTranslation(fileId, translatedName, outputPath);
    ...
    const jobService = new JobService();
    await jobService.updateJobProgress(jobId, "FAILED" as JobStatus, "TRANSLATE" as ProcessStep, 75);
  }
}
